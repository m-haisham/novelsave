...
def test_is_url_true(url):
    assert url_helper.is_url(url), "Was not identified as a url, even though its is valid"
...
def test_is_url_false(url):
    assert not url_helper.is_url(url), "Was identified as url, even thought it isn't valid"
